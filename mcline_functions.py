...
#Reads in data from the reddened fullsed file
def readInReddeningFile(filename, luminosity, distance):
    ...
    #Creates a table in which each row will contain all data points for one of the variables listed above
    for i in range(3):
        data.append([])

    #Reads in each line of data from the file and appends each point to the proper row in the table
    i=-1
    prevMu=-20000
    for line in file:
        line=line.split()
        if (float(line[0].strip())==prevMu):
            data[0][i].append(float(line[0].strip()))
            data[1][i].append(float(line[1].strip()))
            data[2][i].append(float(line[2].strip())*float(line[1].strip())*10**23/10**4/2.99792458/10**10*2.99792458*10**8/(float(line[1].strip()))*10**6/(3.9*10**33*luminosity)*(4*np.pi*(distance*3.08568*10**18)**2)*float(line[1].strip()))
...
#Reads in data from the h alpha file
def readInHalphaFile(fileName):
    ...
    for line in file:
        line=line.rsplit(" ", 1)
        data[0].append(float(line[0].strip()))
        data[1].append(float(line[1].strip()))

    file.close()

    return data
   
#Acquires the observed Halpha data
def plotRiviData():
    
    #REDACTED

    return toPlotx[0], toPloty[0]
...
def findHAlphaLine(wavelength, flux, plotWave, normFlux, subtractCont):
    ...
    velocity=list(map(lambda obs:(obs-h_alpha_wavelength)*c/h_alpha_wavelength, wavelength))
    ...
    #Adjust the index value of the rightmost velocity (it is not included in the [x:x] operation
    if (rightmost<n):
        rightmost+=1

    #Take only the velocities within the desired range
    velocity=velocity[leftmost:rightmost+1] #For some reason the +1 makes it work. Explore this later.
    flux=flux[leftmost:rightmost+1]
    wavelength=wavelength[leftmost:rightmost+1]

    n=len(velocity)

    #For low resolution data around the H_alpha line, make the velocity plot flat (all 1's)
    if(n<3):
        if plotWave:
        ...
        if normFlux:
            ncont=6 #number of points of the continuum that will be used in the fitting
            ...
            normalized_continuum=list(map(lambda x: normal_eqn[0]*x+normal_eqn[1], wavelength))

            #Normalizes the flux
            flux=list(map(lambda x,y: x/y, flux, normalized_continuum))
        
        if subtractCont:
            #This line is for comparing the fluxes of halpha lines with different continuum emissions
...
def findExtremeVelocity(velocity, extremeValue):
    ...
    while (upper-lower>1):
        ...
        #If the endpoint of the range is greater than the middle value, make the middle value the lower bound of the new range
    ...
    #Note that lower contains the INDEX value of the data point that fits the above description
    #Adjust the value of lower accordingly in these special cases so that it is a valid index value
    if (lower==-1):
        lower=0
    elif (lower==n):
        lower=n-1

    #Returns the index value of the data point
    return lower

#Organizes simulation data acquisition, interpolation and convolution
def getSim(filename, inclAngle, ranges, luminosity, distance, fac_e, v_h,
    ...
    data=readInReddeningFile(filename, luminosity, distance)
    ...
    if inclFlag: #If there are a range of inclinations
        ...
        ilow = lowiinrange + math.floor((inclAngle-lowiinrange)/inclSpacing)*inclSpacing
        #ilow = math.floor(inclAngle*(1./inclSpacing))/(1./inclSpacing)
        ihigh = ilow+inclSpacing
        #get the idx of the mu/incl in the fullsed
        idxlow = getMuIdx(data[0], ilow)
        idxhigh = getMuIdx(data[0], ihigh)
        #get the flux at these inclinations
        flow.append([data[1][idxlow], data[2][idxlow]])
        fhigh.append([data[1][idxhigh], data[2][idxhigh]])
        #interpolate the Halpha line at the desired inclination
        flux.append([data[1][idxlow], np.ndarray.tolist(inclInterp(inclAngle, [ilow,ihigh], flow, fhigh))])
        #convolve the flux of the inclination interpolated line
        for values in flux:
            (flux_conv, origData) = gaussconv(values, fac_e, v_h, v_e, plotWave, normFlux, subtractCont)
        
    else: #If just one inclination is given
        inclIndex = getMuIdx(data[0], inclAngle)
        flux.append([data[1][inclIndex],data[2][inclIndex]])
...
def ewcalc(velocity,flux,plotWave,normFlux,subtractCont):
    ...
    c=299792#speed of light in km/s
    h_alpha_wavelength=6564.61#wavelength of H_alpha emission line (angstroms)
    wavelength=list(map(lambda obs: (obs/c)*h_alpha_wavelength+h_alpha_wavelength, velocity))
    ...
    #manipulates data to find h-alpha line
    normFlux=False
    plotWave=True
    ...
    with warnings.catch_warnings():
        ...
        ew = integrate.quad(f,6539.77,6585.86)#integrates f*dlambda with lambda in chris's fixed integration limits (i.e integral((1-flux)dlambda), lambda = 0.653997 to 0. 658586)
...
def writeBestFit(filename, bestIncl, fac_e, v_h, v_e, interpFlux, obsValX, ew_fit,
                bestChi2, bestRedChi2):
    #Make the out folder
    dir0 = os.getcwd()
    savepath = dir0+'/out/'
    ...
    with open(filename, 'w') as file:
        file.write('%'+filename+'\n')
        file.write('%Date Created:'+datetime.datetime.today().strftime('%y-%m-%d %H:%M:%S')+'\n')
        file.write('{0:20}{1:20}{2:20}{3:20}{4:20}{5:20}{6:20} \n'.format('Incl [deg]',\
                 'f_e', 'v_h [km/s]', 'v_e [km/s]', 'EW [nm]', 'chi2', 'redChi2'))
        file.write('{0:20}{1:20}{2:20}{3:20}{4:20}{5:20}{6:20} \n'.format(\
            str('{0:.5f}'.format(bestIncl)),str('{0:.5f}'.format(fac_e)),str('{0:.5f}'.format(v_h)),\
            str('{0:.5f}'.format(v_e)),str('{0:.5f}'.format(ew_fit)),str('{0:.5f}'.format(bestChi2)),\
...
#Plots the best fitting data set simulation data
def pltBestFit(params, inclAngles, ranges, obsValX, obsValY, inclFlag, reddeningFiles,
                luminosity, distance, plotWave, normFlux, subtractCont, filename,
                veloUBound, veloLBound, bestChi2, plotOrigLine, replotting, *inclSpacing):
    if inclFlag:
        bestIncl, fac_e, v_h, v_e = params
        simX, simY, origData = getSim(reddeningFiles[0], bestIncl, ranges,
                        luminosity[0], distance, fac_e, v_h, v_e, inclFlag, plotWave,
                        normFlux, subtractCont, inclSpacing)
    else:
    ...
    interpFlux = griddata(np.array(simX),simY,np.array(obsValX),method='linear',fill_value=1)
    ...
    print('Best Fitting Model - Chi-squared:', str('{0:.2f}'.format(bestChi2 * -1)))

    #Getting the reduced chi-squared
    #Interpolate the flux to match x array of sim to obs
    interpFlux = griddata(np.array(simX),simY,np.array(obsValX),method='linear',fill_value=1)
    #This value doesn't really matter what it is since all chi2 are divided equally by it
    st_dev = 1
    #Cut the velocity range over which the chi-squared is being computed
    obsValXCut, obsValYCut, interpFluxCut = veloWindow(obsValX,obsValY,interpFlux,veloUBound,veloLBound)
    #Compute the reduced chi-squared (degrees of freedom set to length of params)
    bestRedChi2 = (np.sum(((obsValYCut - interpFluxCut)**2 / (st_dev)**2.)))/(len(interpFluxCut)-len(params))
    print('Best Fitting Model - Reduced Chi-squared:', str('{0:.2f}'.format(bestRedChi2)))

    #Write the best fitting line's flux and wavelength to a file if not replotting
    if not replotting:
        writeBestFit(filename, bestIncl, fac_e, v_h, v_e, interpFluxCut, obsValXCut, ew_fit,\
                    bestChi2, bestRedChi2)
    ...
    #Plot the observed data set to compare
    if plotOrigLine:
...
def plotConvergence(npy, filename, ranges, inclFlag):
    ...
    if inclFlag: #If multiple inclinations are given
        ...
        param_to_latex = dict(f_e=r'$f_{e}$', v_h=r'$v_{h} [km/s]$', v_e=r'$v_{e} [km/s]$')
        params = ["f_e", "v_h", "v_e"]
        fig = plt.figure(num=None, figsize=(6, 8), dpi=100, facecolor='w', edgecolor='k')
...
def lnprob(params, obsValX, obsValY, inclAngles, ranges, inclFlag, reddeningFiles,
    ...
    if inside_ranges:
        ...
        simX, simY, origData = getSim(reddeningFiles[0], inclAngles, ranges, luminosity[0],
                            distance, fac_e, v_h, v_e, inclFlag, plotWave, normFlux,
                            subtractCont, inclSpacing)
        ...
        #Cut the velocity range over which the chi-squared is being computed
        obsValX, obsValY, interpFlux = veloWindow(obsValX,obsValY,interpFlux,veloUBound,veloLBound)
...
def run(filename, a_parameter, nwalk, nstep, nburn, fac_e, v_e, v_h, inclFlag,
    ...
    *inclSpacing):
    ...
    with Pool() as pool:
        if inclFlag:
            sampler = emcee.EnsembleSampler(nwalk, ndim, lnprob, args=[obsValX, obsValY, inclAngles,
            ranges, inclFlag, reddeningFiles, luminosity, distance, plotWave, normFlux, subtractCont,
            veloUBound, veloLBound, inclSpacing], a=a_parameter, pool=pool)
        else:
            sampler = emcee.EnsembleSampler(nwalk, ndim, lnprob, args=[obsValX, obsValY, inclAngles,
            ranges, inclFlag, reddeningFiles, luminosity, distance, plotWave, normFlux, subtractCont,
            veloUBound, veloLBound], a=a_parameter, pool=pool)
        ...
        print("--- Completed in %s minutes ---" % (str('{0:.2f}'.format((time.time() - start_time) / 60))))
        af_bi = np.mean(sampler.acceptance_fraction)
        print("Mean acceptance fraction (Burn-In): ", str('{0:.2f}'.format(af_bi)))
        sampler.reset()
    ...
    figure.savefig(savepath+filename+'_af'+str('{0:.2f}'.format(af))+"_corner.png")
    ...
    replotting=False
    if inclFlag:
        pltBestFit(params_fit, inclAngles, ranges, obsValX, obsValY, inclFlag, reddeningFiles,
